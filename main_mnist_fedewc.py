...
def consolidate(Model, dataset, sample_size, batch_size):
    ...
    for x, y in data_loader:
        ...
        y = Variable(y).type(torch.LongTensor).to(device)
        loglikelihoods.append(
                    F.log_softmax(Model(x), dim=1)[range(batch_size), y.data]
                )# self(x) the model's output
    ...
    mean = {n: p.data for n, p in Model.named_parameters()}
    return fisher,mean
...
if __name__ == '__main__':
    ...
    for interation in range(T):
        ...
        for client in range(num_clients):
            ...
            omega_current[client], mean_current[client] = consolidate(Model=net, dataset=idx_traindataset, sample_size=1024,batch_size=batch_size)
            t1 = time.clock()
