...
def FedAvg(model, w_pre, w_update, error):
    w_update_avg = torch.div(torch.sum(torch.stack(w_update), dim=0), len(w_update))
    # w_avg_vec = w_update_avg + w_pre
    # w_avg_vec = topk(vec=(w_update_avg + w_pre), k=int(rho * len(w_pre)))

    w_update_avg_spar, index = topk(vec=(w_update_avg), k=int(rho * len(w_pre)))
    w_avg_vec = w_pre + w_update_avg_spar
    # w_avg_vec = w_pre - w_update_avg_spar

    for i,s in enumerate(w_update):
        s0 = torch.zeros_like(s)
        s0[index] = s[index]  ### s0 sparisified
        error[i] = error[i] + (s0 - s)

    w_avg = model.state_dict()
    vec = w_avg_vec.to(device)
    ...
    for k in w_avg.keys():
        ...
        w_avg[k] = vec[startPos:startPos + size].reshape(shape)
        startPos += size
    return w_avg, error
...
def sparsity(fisher, w_update, w_prev, topkIndices,s):
    Shapes = []
    Sizes = []
    ...
    fisher_vector_spar_q = quantize(fisher_vector_spar,s)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--E', type=int, nargs='?', default=5, help="local epoches")
    parser.add_argument('--rho', type=float, nargs='?', default=0.5, help="sparsification ratio")
    parser.add_argument('--s', type=int, nargs='?', default=32, help="quantization")
    ...
    for interation in range(T):
        w_vec_locals, loss_locals = [], []
        # print('interationh',interation)
        weight_vec_pre = getGradVec(w_glob)
        for client in range(num_clients):
            ...
            for epoch in range(1, epochs_per_task + 1):
                ...
                for batch_idx, (images, labels) in enumerate(ldr_train):
                    ...
                    grad_params = torch.autograd.grad(ce_loss, net.parameters(), create_graph=True)
                    if interation == 0:
                        ewc_loss = torch.FloatTensor([0.0]).to(device) #torch.DoubleTensor
                    else:
                        other_clients = list(set([i for i in range(num_clients)])-set([client]))
                        losses = {}
                        sum_ewc_loss = 0
                        for i in other_clients:
                            losses[i] = []
                            for n, p in net.named_parameters():
                                mean, omega = Variable(mean_current[i][n]), Variable(omega_current[i][n])
                                losses[i].append((omega * (p - mean) ** 2).sum())
                            sum_ewc_loss += sum(losses[i])
                        ewc_loss = net.lamda * sum_ewc_loss
                    loss = ce_loss + ewc_loss.double()  # + reg_loss.double()
                    pred = scores.max(1)[1]
                ...
                # if (interation != 0) and (client == 0):
                #     print('1', sum(losses1), sum(lossnon), '2', sum(losses2), '3', sum(losses3), '4', sum(losses4))
            ...
            K = int(rho * len(weight_vec_current))
            print('sparsity k=', K)

            if interation == 0:
                error_compensation[client] = torch.zeros_like(weight_vec_current)
            weight_update, Topkindices = topk(vec=(weight_vec_current - weight_vec_pre + error_compensation[client]),k=K)
            weight_update_q = quantize(weight_update,s)
            error_compensation[client] = (weight_update_q - (weight_vec_current - weight_vec_pre + error_compensation[client]))  # + error_compensation0
            omega_current_00, mean_current_00 = consolidate(Model=net, Weight=W, MEAN_pre=mean_pre, epsilon=epsilon)
            omega_current[client], mean_current[client] = sparsity(fisher=omega_current_00, w_update=weight_update_q,w_prev=weight_vec_pre, topkIndices=Topkindices,s=s)
            w_vec_locals.append(weight_update_q)
            t1 = time.clock()
            print('client:\t', client, 'trainingtime:\t', str(t1 - t0))
            # weight_update, _ = topk(vec=(weight_vec_current - weight_vec_pre + error_compensation), k=K)
            # weight_update_q = quantize(weight_update)
            # # weight_update_q=quantize_log(weight_update)
            # # weight_update_q=quantization_layer(sizes=torch.tensor([144, 16, 4608, 32, 2560, 5]),x=weight_update)
            # error_compensation = error_compensation + (
            #         weight_vec_current - weight_vec_pre + error_compensation - weight_update_q)
            # w_vec_locals.append(weight_update_q)

            # weight_update, _ = topk(vec=(weight_vec_current - weight_vec_pre), k=K)
            # # weight_update_q=quantize(weight_update)
            # # weight_update_q=quantize_log(weight_update)
            # # weight_update_q=quantization_layer(sizes=torch.tensor([144, 16, 4608, 32, 2560, 5]),x=weight_update)
            # w_vec_locals.append(weight_update)

        # w_glob = FedAvg(model=copy.deepcopy(net_global), w_pre=weight_vec_pre, w_update=w_vec_locals)
        w_glob,error_compensation = FedAvg(model=copy.deepcopy(net_global), w_pre=weight_vec_pre, w_update=w_vec_locals, error=error_compensation)
