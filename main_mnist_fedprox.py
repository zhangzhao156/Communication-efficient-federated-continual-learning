        ...
        for client in range(num_clients):
            ...
            for epoch in range(1, epochs_per_task + 1):
                ...
                for batch_idx, (images, labels) in enumerate(ldr_train):
                    ...
                    if interation == 0:
                        prox_loss = torch.DoubleTensor([0.0]).to(device)
                    else:
                        loss_prox = []
                        for n, p in net.named_parameters():
                            mean = Variable(mean_pre[n])
                            loss_prox.append(((p - mean) ** 2).sum())
                            sum_prox_loss = sum(loss_prox)
                            prox_loss = net.lamda * sum_prox_loss
                    loss = ce_loss + prox_loss  # .double()
